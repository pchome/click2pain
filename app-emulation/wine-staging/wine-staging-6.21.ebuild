...
LICENSE="LGPL-2.1"
SLOT="${MY_PV}"
IUSE="+abi_x86_32 +abi_x86_64 +alsa capi cups custom-cflags dos elibc_glibc +fontconfig +gecko gphoto2 gssapi gstreamer kerberos kernel_FreeBSD ldap mingw +mono mp3 netapi nls odbc openal opencl +opengl osmesa oss +perl pcap pipelight prelink proton pulseaudio +realtime +run-exes samba scanner sdl selinux +ssl staging test +threads +truetype udev +udisks +unwind usb v4l vkd3d vulkan +X +xcomposite xinerama"
...
src_prepare() {
	...
	# NOTE: Proton patches may contain changes for
	# include/wine/server_protocol.h, server/trace.c, server/request.h, etc.,
	# which are generated ones (e.g. from server/protocol.def)
	if use proton; then
		## NOTE: Prepare sources for patching
		## Modification of the server protocol requires regenerating the server requests
		#if [[ "$(md5sum server/protocol.def)" != "${md5}" ]]; then
		#       einfo "server/protocol.def was patched; running tools/make_requests"
		#       tools/make_requests || die #432348
		#       # Save new md5
		#       md5="$(md5sum server/protocol.def)"
		#fi

		# TODO: add Proton patches here
		# Required minimum for Proton:
	...
	if [ ${cores:-8} -lt 8 ]; then
		eapply "${FILESDIR}/octa-core.patch"
	fi
